...
WIDESURF_SEMAPHORE = asyncio.Semaphore(5)  # Global limit for Widesurf API calls
DB_SEMAPHORE = asyncio.Semaphore(20)       # Limit for parallel DB operations
...
# Database connection helper (imported from database.py)
# get_db_conn is now imported



# Database utility functions are now imported from database.py
...
def fetch_and_cache_prices(symbol: str, start_date: str, end_date: str, timeframe: str = "1Day") -> list:
    ...
    for fetch_start, fetch_end in fetch_ranges:
        ...
        if timeframe == "1Day":
            ...
            # Fetch intraday data in weekly chunks (API now paginates to get ALL bars)
                ...
                try:
                    ...
                    if resp.status_code == 200:
                        ...
                        if bars and len(bars) > 0:
                            # Filter out daily bars that sometimes appear in minute response
                            # Daily bars have timestamps at 05:00:00 UTC (market open aggregation)
                            # Intraday bars have varying hours (pre-market starts at 08:00 UTC / 4AM ET)
                            intraday_bars = []
                            for b in bars:
                                t = b.get("t", "")
                                # Parse hour from ISO timestamp like "2024-01-02T14:30:00Z"
                                if "T" in t:
                                    try:
                                        hour = int(t.split("T")[1].split(":")[0])
                                        # Real intraday bars have hours between 08-21 UTC (4AM-4PM ET)
                                        # Daily aggregate bars have T05:00:00Z (midnight ET)
                                        if hour != 5:
                                            intraday_bars.append(b)
                                    except (ValueError, IndexError):
                                        intraday_bars.append(b)  # Keep if unparseable
                            
                            if intraday_bars:
                                save_prices_to_cache(symbol, timeframe, intraday_bars)
                                logger.info(f"Cached {len(intraday_bars)} intraday bars for {symbol} ({current.strftime('%Y-%m-%d')} to {chunk_end.strftime('%Y-%m-%d')})")
                except Exception as e:
...
def fetch_prices_from_widesurf(symbol: str, start_date: str, end_date: str, timeframe: str = "1Min", target_time: str = None) -> list:
    ...
    Args:
        ...
        target_time: Optional time suffix (e.g., "1000" for 10:00 AM) to get aggregates up to that time
    ...
    # Format: GET /v1/aggs/ticker/{ticker}/range/{multiplier}/{timespan}/{from}/{to}
...
async def get_widesurf_tickers(keyword: str):
    ...
    if not url or not url.startswith("http"):
        default_urls = {
            'SPY500': f"http://10.0.0.94:1020/v1/reference/tickers?market=stocks&active=true&order=desc&limit=500&sort=volume&type=limited&apiKey={WIDESURF_API_KEY}",
            'NASDAQ500': f"http://10.0.0.94:1020/v1/reference/tickers?market=stocks&active=true&order=desc&limit=500&sort=volume&type=limited&apiKey={WIDESURF_API_KEY}",
            '1000': f"http://10.0.0.94:1020/v1/reference/tickers?market=stocks&active=true&order=desc&limit=1000&sort=volume&type=limited&apiKey={WIDESURF_API_KEY}",
            '10000': f"http://10.0.0.94:1020/v1/reference/tickers?market=stocks&exchange=XNAS%2C+NYSE&active=true&order=desc&limit=10000&sort=transactions&type=limited&apiKey={WIDESURF_API_KEY}"
        }
...
def get_default_keyword_configs():
    ...
    return {
        ...
        "10000": {
            "api_url": f"http://10.0.0.94:1020/v1/reference/tickers?market=stocks&exchange=XNAS%2C+NYSE&active=true&order=desc&limit=10000&sort=transactions&type=limited&apiKey={WIDESURF_API_KEY}",
...
def generate_trade_log(bars: list, buy_trigger_pct: float, sell_trigger_pct: float, compound: bool, capital: float = 100000, algo: str = "default", symbol: str = None, start_date: str = None, end_date: str = None, stop_loss_pct: float = None, trailing_stop_pct: float = None, data_source: str = "alpaca") -> list:
    ...
    # For ChatGPT algos, try to fetch 10AM prices (using cache or API)
    price_9am_map = {}
    
    # NEW: Check if bars already have the 10AM data to avoid redundant heavy downloads
    has_enriched_data = False
    if bars and len(bars) > 0:
        first_bar = bars[0]
        if "price_10am" in first_bar or "vwap" in first_bar:
            has_enriched_data = True
            logger.info(f"Using pre-enriched data for {symbol} trade log (skipping re-fetch)")

    if not has_enriched_data and algo in ("chatgpt", "chatgpt_stoploss", "chatgpt_vwap", "chatgpt_vwap_rust") and symbol and start_date and end_date:
        try:
            ...
            if data_source == "widesurf":
                ...
                for bar in bars_1000:
                    ...
                    p10 = bar.get("c", 0)
                    vw = bar.get("vw", p10)
                    vw940 = vwap_940_lookup.get(date_key, vw)
                    oh = bar.get("h", p10)
                    ol = bar.get("l", p10)
                    
                    price_9am_map[date_key] = {
                        "price_10am": p10,
                        "vwap": vw,
                        "vwap_940": vw940,
                        "or_high": oh,
                ...
                # Use cached intraday data (Alpaca/Legacy)
...
async def optimize_stock(
    ...
    # If engine_override is set, we follow that. Otherwise follow config.use_gpu.
    use_gpu_path = (config.use_gpu and GPU_BACKTEST_AVAILABLE)
    if engine_override == 'gpu': use_gpu_path = True
    if engine_override == 'cpu': use_gpu_path = False

    logger.info(f"Optimization Path Decision for {symbol}: use_gpu_path={use_gpu_path}, engine_override={engine_override}")
    
    # We enter this block if CPU-Parallel or GPU is requested
    if use_gpu_path or getattr(config, 'algo', '') == 'chatgpt_vwap_rust':
        await broadcast_message({
            "type": "status",
            "job_id": job_id,
            "symbol": symbol,
        ...
        try:
            ...
            if algo_type == "chatgpt":
                ...
                if data_source == "widesurf":
                    ...
                    if bars_1000:
                        try:
                            ...
                            logger.warning(f"Time-sampled 9:40AM API failed for {symbol}: {e}")
                    ...
                    for bar in bars_1000:
                        date_part = bar["t"][:10]
                        price_10am = bar.get("c", 0)
                        vwap_to_10am = bar.get("vw", price_10am)
                        vwap_940 = vwap_940_lookup.get(date_part, vwap_to_10am)
                        or_high = bar.get("h", price_10am)
                        or_low = bar.get("l", price_10am)
                ...
                # When time-sampled data is missing, estimate price_10am from daily bars:
                #   - Use daily VWAP (vw field) as the VWAP reference
                #   - Estimate price_10am as open + 30% of daily range (typical by 10AM)
                bars_with_vwap = []
                enriched_from_daily = 0
                for bar in bars:
                    bar_date = bar.get("t", "")[:10] if bar.get("t") else ""
                    new_bar = dict(bar)
                    vwap_info = vwap_data_map.get(bar_date, {})
                    
                    if vwap_info and vwap_info.get("price_10am", 0) > 0:
                        # Got real intraday data
                        new_bar["price_10am"] = vwap_info["price_10am"]
                        new_bar["vwap"] = vwap_info.get("vwap", bar.get("vw", bar.get("c", 0)))
                        new_bar["vwap_940"] = vwap_info.get("vwap_940", new_bar["vwap"])
                        new_bar["or_high"] = vwap_info.get("or_high", bar.get("h", 0))
                        new_bar["or_low"] = vwap_info.get("or_low", bar.get("l", 0))
                    else:
                        # Estimate from daily bar — use VWAP and price estimate
                        o = float(bar.get("o", 0))
                        h = float(bar.get("h", 0))
                        l = float(bar.get("l", 0))
                        c = float(bar.get("c", 0))
                        ...
                        # Estimate 10AM price as open + 30% of (close - open)
                        # This approximates ~30 min of morning movement in a 6.5hr trading day
                        if o > 0:
                            price_10am_est = o + 0.3 * (c - o)
                            # Clamp to daily range
                            price_10am_est = max(l, min(h, price_10am_est))
                        else:
                            price_10am_est = c
                        
                        new_bar["price_10am"] = price_10am_est
                        new_bar["vwap"] = vw if vw > 0 else (o + h + l + c) / 4
                        new_bar["vwap_940"] = new_bar["vwap"]
                        new_bar["or_high"] = o + 0.3 * (h - o) if o > 0 else h
                        new_bar["or_low"] = o + 0.3 * (l - o) if o > 0 else l
                        enriched_from_daily += 1
                    
                    bars_with_vwap.append(new_bar)
                
                if enriched_from_daily > 0:
                    logger.warning(f"{symbol}: Estimated 10AM data from daily bars for {enriched_from_daily}/{len(bars)} days (time-sampled API unavailable)")
                ...
                if use_gpu_path:
                    ...
                    logger.info(f"[Hybrid] Dispatching {symbol} to CPU Engine (Rust)")
                    # If this is the Rust algorithm, we can mention CPU cores
                    await broadcast_message({
                        "type": "status",
                        "job_id": job_id,
                        "symbol": symbol,
                        "status": "optimizing_cpu",
                ...
                # Keep enriched bars for trade log generation
                bars = bars_with_vwap
            else:
            ...
            result = {
                ...
                "method": "gpu_grid_search" if use_gpu_path else "cpu_parallel",
                "gpu_enabled": use_gpu_path,
            ...
            # Recalculate metrics from actual trade log (matches backtest table)
            trade_log = result.get("trade_log", [])
            if trade_log:
                bought_trades = [t for t in trade_log if t.get("bought")]
                winning = [t for t in bought_trades if (t.get("profit") or 0) > 0]
                bt_win_rate = len(winning) / len(bought_trades) if bought_trades else 0
                last_equity = trade_log[-1].get("equity", config.capital)
                bt_total_return = (last_equity - config.capital) / config.capital
                result["metrics"]["win_rate"] = round(bt_win_rate, 4)
                result["metrics"]["total_return"] = round(bt_total_return, 4)
                result["metrics"]["final_equity"] = round(last_equity, 0)
            ...
            try:
                ...
                if legacy_row and legacy_row[0]:
                    cache_info.append({
                        ...
                        "min_date": legacy_row[0].isoformat(),
                        "max_date": legacy_row[1].isoformat(),
